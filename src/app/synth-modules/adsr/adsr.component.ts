...
export class ADSRComponent implements OnInit {
  ...
  public selectPoint(e){
    //console.log("clicked");
    const pos = {
      x: e.clientX - this.envCanvas.nativeElement.offsetLeft - this.offsetParentLeft + this.scrollOffsetX,  //10 is page offset, has to be changed
      y: e.clientY - this.envCanvas.nativeElement.offsetTop - this.offsetParentTop + this.scrollOffsetY    //same
  ...
  public movePoint(e){
    ...
    if(!this.flagDown){
      ...
      let mouseX=e.clientX-this.offsetX-this.offsetParentLeft + this.scrollOffsetX;
      let mouseY=e.clientY-this.offsetY-this.offsetParentTop + this.scrollOffsetY;
      var dx=mouseX-this.lastX[this.selectedPoint];
      var dy=mouseY-this.lastY[this.selectedPoint];
