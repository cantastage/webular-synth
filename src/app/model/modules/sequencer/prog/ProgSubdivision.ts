export class ProgSubdivision {
    ...
    public set duration(duration: number) {
        if (duration < ProgSubdivision.DURATION_MIN || duration > ProgSubdivision.DURATION_MAX) {
    ...
    public set velocity(velocity: number) {
        if (!Number.isInteger(Number(velocity)) || velocity < ProgSubdivision.VELOCITY_MIN || velocity > ProgSubdivision.VELOCITY_MAX) {
    ...
    public static generateOctaveVector(octaveCount: number): number[] {
        ...
        for (let i = 0; i < octaveCount; i++) {
          ret.push(ProgSubdivision.OCTAVE_DEFAULT);
        }
        return ret;
    }
}
